...
Only push the current branch to its configured upstream branch if the branch names match; fail otherwise.

```shell
git config --global push.default single
```
...
Log the new commits on the specified local branch from where it diverges from the current local branch.

```shell
git log --first-parent <local_branch_name>
```

_NOTE: The <local_branch_name> parameter is the name of the branch whose commits should be logged._
